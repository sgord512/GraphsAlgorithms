define(["lib/utilities", "deps/under"], function(utilities, underscore) {
    ...
    Graph.prototype.contract_edge = function() {
        ...
        this.edges = _.reject(this.edges, function(e) { return (e.v === edge.v) && (e.u === edge.v); });
        ...
        this.vertices = _.without(this.vertices, removed);
    }

    Graph.prototype.kargers_algorithm = function() { 
        var self = this;
        _((this.vertices.length * (this.vertices.length - 1) * c) / 2).times(function() { self.add_min_cuts(); });
        var min_cuts = _.groupBy(this.min_cuts, 'cost');
        var min_cost = _.min(_.keys(min_cuts));
        return _.first(min_cuts[min_cost]);
    }

    Graph.prototype.do_iteration = function() {
        this.original.edges = _.map(this.edges, function(e) { return new Edge(e.u,e.v); });
        this.original.vertices = _.map(this.vertices, function(v) { return new Vertex(v.index); });
        while(this.vertices.length > 2) {
            this.contract_edge();
        }
        var self = this;
        var min_cuts = _.map(this.vertices, function(v) {
    ...
    var vertices = _.map(_.range(0,10), function(i) { return new Vertex(i); });
